...
seed = 201; randn('seed',seed), rand('seed',seed)
...
for i=1:size(id,1) %for each method/inference pair, save output of gp
  lik = lik_list{id(i,1)};                                % setup the likelihood
  if strcmp(lik,'likT')
    nu = 4;
    hyp0.lik  = log([nu-1;sqrt((nu-2)/nu)*sn]); %initialize hyperparameters
  else
    hyp0.lik  = log(sn);
  end
  ...
  [ymu{i+1}, ys2{i+1}] = gp(hyp, inf, mean, cov, lik, xtr, ytr, xte);  % predict on xte
  [nlZ(i+1), dnlZ] = gp(hyp, inf, mean, cov, lik, xtr, ytr);
end
