...
%   z        ---Within covFcns means values for comparison, or diag option
%
...
try                                                  % call the inference method
  ...
  if nargin>7   % compute marginal likelihood and its derivatives only if needed, - ie, not here
    if isstruct(y)
      post = y;            % reuse a previously computed posterior approximation
    else
      post = inf(hyp, mean, cov, lik, x, y);
    end
  else %but here, goes to varargout
    if nargout<=1
      [post nlZ] = inf(hyp, mean, cov, lik, x, y); dnlZ = {};
    else
      [post nlZ dnlZ] = inf(hyp, mean, cov, lik, x, y); %----------called here
    end
  end
catch
  msgstr = lasterr;
...
if nargin==7                                     % if no test cases are provided
  varargout = {nlZ, dnlZ, post};    % report -log marg lik, derivatives and post, return
else
