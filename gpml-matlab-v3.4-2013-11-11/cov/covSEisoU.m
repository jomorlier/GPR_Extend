...
xeqz = numel(z)==0; dg = strcmp(z,'diag') && numel(z)>0;        % determine mode - diagonal?

ell = exp(hyp(1));                                 % characteristic length scale, l

% precompute squared distances, first part of the cov func
if dg                                                            % vector kxx
  K = zeros(size(x,1),1);
else
